...
var loginUserIntoDB = async (payload) => {
  ...
  const data = { token: accessToken, user };
  return data;
};
var authService = {
  loginUserIntoDB,
...
var validateIssueDescription = (description) => {
...
${JSON.stringify(data, null, 2)}`;
  ...
  // Request logging
  static logRequest(method, path2, statusCode) {
    const status = statusCode ? ` \u2192 ${statusCode}` : "";
    this.info(`${method} ${path2}${status}`);
  }
  ...
  // Auth event logging
  static logAuth(event, userId, email) {
    this.info(`Auth Event: ${event}`, { userId, email });
  }
  // Issue event logging
  static logIssue(event, issueId, userId) {
...
var errorHandlingMiddleware = (error, req, res, next) => {
  let statusCode = 500;
...
var calculatePagination = (page = 1, limit = 10) => {
...
var getIssueByIdForOwnerCheck = async (id) => {
  const result = await pool.query(
    `SELECT * FROM issues WHERE id = $1`,
    [id]
  );
...
var getAllIssues2 = asyncHandler(
  async (req, res) => {
...
var getSingleIssue = asyncHandler(
  async (req, res) => {
...
issuesRoute.post("/", auth, createIssue2);
issuesRoute.get("/", getAllIssues2);
issuesRoute.get("/:id", getSingleIssue);
issuesRoute.patch("/:id", auth, updateIssue2);
issuesRoute.delete("/:id", auth, deleteIssue2);
